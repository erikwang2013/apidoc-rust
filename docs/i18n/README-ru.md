...
- Онлайн-отладка (браузер подключается к целевому API напрямую через CORS), Mock-данные (генерация по правилам fake)
- Несколько приложений / несколько версий / пароль доступа
- Экспорт в Markdown / TypeScript / Swagger (OpenAPI3)
- Адаптеры для разных фреймворков (apidoc-axum / apidoc-actix)
...
│   ├── apidoc-macros/         # proc-macro: 7 атрибутных макросов
...
| M2 | Адаптер axum + встроенный UI документации + группировка по разделам | ⏳ В планах |
| M3 | Дополнение аннотаций (tag/group/author/header/route_param/response_status/success/error/not_debug/md/sort/ref) | В планах |
| M4 | Онлайн-отладка + движок Mock | В планах |
